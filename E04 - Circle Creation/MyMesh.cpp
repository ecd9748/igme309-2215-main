...
void MyMesh::GenerateCircle(float a_fRadius, int a_nSubdivisions, vector3 a_v3Color)
{
	...
	if (a_nSubdivisions > 360)
		a_nSubdivisions = 360;
	//vector to hold the points around the circle
	std::vector<vector3> vertexes;
	//variables for the angle and change in angle
	GLfloat angle = 0;
	GLfloat angleChange = static_cast<GLfloat>(2.0 * PI / static_cast<GLfloat>(a_nSubdivisions));
	//loop to create the points for the circle that makes the subdivision amount of points
	for (int i = 0; i < a_nSubdivisions; i++)
	{
		vertexes.push_back(vector3(cos(angle) * a_fRadius, sin(angle) * a_fRadius, 0.0f));
		//increases the angle to travel around the circle
		angle += angleChange;
	}
	//loop through the subdivisions creating each triangle to create the circle
	for (int i = 0; i < a_nSubdivisions; i++)
	{
